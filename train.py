...
from model import model_rrdb, model_swinir, fsgan
...
from model.Discriminator_se import Discriminator as Discriminator_se
from model.Discriminator_se import calculate_loss_D, calculate_loss_G

import torch.nn.functional as F

def parse_args():
    parser = argparse.ArgumentParser(description='Training')
    parser.add_argument('--opt', type=str, default='path/to/train_rrdb_P+FSGAN.yml')
    parser.add_argument(
        "--resume",
        type=str
    )
    ...
    parser.add_argument('--data_test_root', type=str, default='path/to/Set5')
...
def loss_function(seg_sr, seg_hr):
    loss = 0
    distill_weight = 10.0
    batch_size = seg_sr.size(0)
    distill_loss_all = 0.
    n_valid_samples = 0


    seg_sr_reshaped = seg_sr[0].view(-1, seg_sr.size(1))
    seg_hr_reshaped = seg_hr[0].view(-1, seg_hr.size(1))
    seg_sr_log = torch.log(seg_sr_reshaped + 1e-8)

    for idx_in_batch in range(batch_size):
        distill_loss = F.kl_div(
            seg_sr_log, seg_hr_reshaped, reduction='batchmean')
        distill_loss_all += distill_loss
        loss = loss + torch.mean(distill_weight * distill_loss)
        n_valid_samples += 1
...
def main():
    ...
    model_ex = fsgan.CLIP_Semantic_extractor(**opt['model_ex']).to('cuda')

    if opt['name'].split('_')[-1] == 'P+FSGAN':
        model_d = fsgan.FSGAN_P(**opt['model_d']).to('cuda')
    ...
    loss_p = losses.PerceptualLoss(layer_weights=loss_dict_per, perceptual_weight=loss_weight['loss_p'],
                                   criterion='l1').to('cuda')
    ...
    for epoch in range(total_epochs):
        ...
        for lr, hr, filename in data_loader:
            ...
            # gan loss
            hr_seg_real, sr_seg_fake = calculate_loss_D(hr, sr, seg_map_sr)
            hr_seg_semantic = model_ex(hr_seg_real)

            fake_g_pred_glo = model_d(sr, hr_semantic)
            loss_gan_glo = loss_g(fake_g_pred_glo, True, is_disc=False)
            loss_gan = 0.5 * loss_gan_loc + 0.5 * loss_gan_glo
            ...
            # real
            real_d_pred_glo = model_d(hr, hr_semantic)
            l_d_real_glo = loss_g(real_d_pred_glo, True, is_disc=True)
            l_d_real = 0.5 * l_d_real_loc + 0.5 * l_d_real_glo

            loss_dict['l_d_real'] = l_d_real.item()
            # l_d_real.backward()
            # fake
            fake_d_pred_glo = model_d(sr.detach().clone(), hr_semantic)  # clone for pt1.9
            l_d_fake_glo = loss_g(fake_d_pred_glo, False, is_disc=True)
            l_d_fake = 0.5 * l_d_fake_loc + 0.5 * l_d_fake_glo
            loss_dict['l_d_fake'] = l_d_fake.item()
            # l_d_fake.backward()


            loss_D = (l_d_real + l_d_fake) / 2
            # loss1 = loss_D.detach_().requires_grad_(True)
            loss_D.backward()
            ...
            if current_step % opt['train']['log_every'] == 0 and torch.cuda.current_device() == 0:
                logger.info(
                    'LR: {} | Step: {} | loss_pix: {:.3f} | loss_per: {:.3f} | loss_gan: {:.5f} | loss_d_real: {:.3f} | loss_d_fake: {:.3f}'.format(
                        learning_r,
                        current_step,
                        loss_dict['loss_pix'],
                        loss_dict['loss_p'],
            ...
            if use_eval and current_step % opt['train']['test_every'] == 0 and torch.cuda.current_device() == 0:
                ...
                for lr, hr, filename in data_loader_test:
                    ...
                    p += psnr
                    s += ssim
                    logger.info('{}: PSNR: {}, SSIM: {}'.format(filename, psnr, ssim))
                p /= count
                s /= count
