...
class Train(data.Dataset):
    def __init__(self, scale=4, patch_size=64, data_root='./data'):
        self.scale = scale
        self.patch_size = patch_size

        # self.dir_hr = os.path.join(data_root, 'DIV2K_train_HR')
        # self.dir_lr = os.path.join(data_root, 'DIV2K_train_LR_bicubic/X4')
        self.dir_hr = os.path.join(data_root, 'FHR')
        self.dir_lr = os.path.join(data_root, 'FX4')
    ...
    def __getitem__(self, idx):
        ...
        hr = hr[croph * self.scale: croph * self.scale + self.patch_size * self.scale,
             cropw * self.scale: cropw * self.scale + self.patch_size * self.scale, :]
...
class Test(data.Dataset):
    def __init__(self, data_root='./Evaluation'):
        ...
        self.dir_hr = os.path.join(data_root, 'image_SRF_4/HR')
        self.dir_lr = os.path.join(data_root, 'image_SRF_4/LR')
        self.images_hr = sorted(
            glob.glob(os.path.join(self.dir_hr, '*.png'))
        )
        self.images_lr = sorted(
