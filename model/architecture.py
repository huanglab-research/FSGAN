...
class Discriminator_VGG_128(nn.Module):
    def __init__(self, in_nc, base_nf, norm_type='batch', act_type='leakyrelu', mode='CNA'):
        ...
        # features
        conv10 = B.conv_block(3, base_nf, kernel_size=3, norm_type=None, act_type=act_type, mode=mode)  # in_nc 改为 512

        conv0 = B.conv_block(in_nc, base_nf, kernel_size=3, norm_type=None, act_type=act_type, mode=mode)  # in_nc 改为 512
        ...
        self.features_3 = B.sequential(conv10, conv1, conv2, conv3, conv4, conv5, conv6, conv7, conv8, conv9)
    ...
    def forward(self, x):
        # x = x.view(x.size(0), x.size(1), 1, 1)  # 将 (3, 512) 转变为 (3, 512, 1, 1)
...
def fc_block(in_features, out_features, norm_type=None, act_type='leakyrelu'):
...
class Discriminator_VGG_Modified(nn.Module):
    def __init__(self, in_nc=768, base_nf=64, norm_type='batch', act_type='leakyrelu'):
        super(Discriminator_VGG_Modified, self).__init__()

        # 使用全连接层处理特征
        self.fc1 = fc_block(768, base_nf, norm_type=norm_type, act_type=act_type)
        self.fc2 = fc_block(base_nf, base_nf * 2, norm_type=norm_type, act_type=act_type)

        # 分类器
        self.classifier = nn.Sequential(
            nn.Linear(base_nf * 2, 100),  # 输入特征数根据上一个全连接层输出调整
            nn.LeakyReLU(0.2, True),
            nn.Linear(100, 1)  # 输出一个标量
        )

    def forward(self, x):
        # 确保输入形状适配 (batch_size, 512)
        x = self.fc1(x)  # 通过第一个全连接层
        x = self.fc2(x)  # 通过第二个全连接层
        x = self.classifier(x)  # 最终分类器
        return x
