...
import sys
sys.path.append('/home/haida/d_j/Semantic-Aware-Low-Light-Image-Enhancement-main')
from DRBN_SKF.src.utils import losses
...
def calculate_loss_D(model_D, input_high, out, seg_map):
    ...
    loss_g = losses.GANLoss(gan_type='vanilla', loss_weight=0.01).to('cuda')
    ...
    real_loss = loss_g(real_label, True, is_disc=True)
    fake_loss = loss_g(fake_label, False, is_disc=True)
    fake_loss_max_1 = loss_g(fake_label_max_1, False, is_disc=True)
    fake_loss_max_2 = loss_g(fake_label_max_2, False, is_disc=True)
    real_loss_max_1 = loss_g(real_label_max_1, True, is_disc=True)
    real_loss_max_2 = loss_g(real_label_max_2, True, is_disc=True)
    ...
    D_loss = lamda * (fake_loss + real_loss) / 2 \
             + (1 - lamda) * (fake_loss_max_1 + real_loss_max_1) / 2
    return fake_loss_max_1, real_loss_max_1
...
# for i in range(N):
    #     # 将当前样本的分割结果移动到 CPU 并转换为 numpy 数组
    #     max_1_seg_np = max_1_seg[i].cpu().squeeze().numpy()
    #     max_2_seg_np = max_2_seg[i].cpu().squeeze().numpy()
    #
    #     # 创建一个包含 1 行 2 列的子图布局
    #     fig, axes = plt.subplots(1, 2, figsize=(10, 5))
    #
    #     # 显示当前样本的第一主要类别分割结果
    #     im1 = axes[0].imshow(max_1_seg_np, cmap='viridis')
    #     axes[0].set_title(f'Sample {i + 1}: Max 1 Segmentation')
    #     axes[0].axis('off')
    #
    #     # 显示当前样本的第二主要类别分割结果
    #     im2 = axes[1].imshow(max_2_seg_np, cmap='viridis')
    #     axes[1].set_title(f'Sample {i + 1}: Max 2 Segmentation')
    #     axes[1].axis('off')
    #
    #     # 添加颜色条作为类别标签
    #     fig.colorbar(im1, ax=axes[0], label='Class Probability')
    #     fig.colorbar(im2, ax=axes[1], label='Class Probability')
    #
    #     plt.tight_layout()
    #     plt.show()