...
import DRBN_SKF.src.model.architecture as arch
...
def define_D(opt):
    opt_net = opt['model_d_l']


    netD = arch.Discriminator_VGG_128(in_nc=opt_net['input_nc'], base_nf=opt_net['nf'],
            norm_type=opt_net['norm_type'], mode=opt_net['mode'], act_type=opt_net['act_type'])


    return netD
class FSGAN_P(nn.Module):
...
class patchGAN(nn.Module):
    def __init__(self, input_nc, ndf=64, use_bias=True):
        """Construct a PatchGAN discriminator
        Parameters:
            input_nc (int)  -- the number of channels in input images
            ndf (int)       -- the number of filters in the last conv layer
        ...
        self.lrelu = nn.LeakyReLU(0.2, True)
        self.conv_first = nn.Conv2d(input_nc, ndf, kernel_size=kw, stride=2, padding=padw)
        self.conv_0 = nn.Conv2d(62, ndf, kernel_size=kw, stride=2, padding=padw)

        self.conv1 = norm(nn.Conv2d(ndf * 1, ndf * 2, kernel_size=kw, stride=2, padding=padw, bias=use_bias))
        self.conv11 = norm(nn.Conv2d(ndf * 2 , ndf * 2, kernel_size=3, stride=1, padding=padw, bias=use_bias))
        self.conv2 = norm(nn.Conv2d(ndf * 2, ndf * 4, kernel_size=kw, stride=2, padding=padw, bias=use_bias))
        self.conv21 = norm(nn.Conv2d(ndf * 4, ndf * 4, kernel_size=3, stride=2, padding=padw, bias=use_bias))

        self.conv3 = norm(nn.Conv2d(ndf * 4, ndf * 8, kernel_size=kw, stride=1, padding=padw, bias=use_bias))
        self.conv31 = norm(nn.Conv2d(ndf * 8, ndf * 8, kernel_size=3, stride=1, padding=padw, bias=use_bias))
    ...
    def forward(self, input):
        ...
        input = self.conv1(input)
        input = self.lrelu(self.conv11(input))

        input = self.conv2(input)
        input = self.lrelu(self.conv21(input))

        input = self.conv3(input)
        input = self.lrelu(self.conv31(input))
